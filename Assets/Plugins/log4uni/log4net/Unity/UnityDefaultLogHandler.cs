    ...
    public class UnityDefaultLogHandler : ILogHandler
    {
        ...
        internal static void UpdateLogHandler()
        {
            ...
            LogLog.LogReceived += (source, args) =>
            {
                ...
                {
                    unityLogHandler.LogFormat(LogType.Warning, null,
                        $"{prefix}{args?.LogLog?.Message}{(args?.LogLog?.Exception != null ? " Exception: " + args.LogLog.Exception.ToString() : "")}");
                }
                else if (prefix.ToLower().Contains("error"))
                {
                    unityLogHandler.LogFormat(LogType.Error, null,
                        $"{prefix}{args?.LogLog?.Message}{(args?.LogLog?.Exception != null ? " Exception: " + args.LogLog.Exception.ToString() : "")}");
                }
                else
                {
                    unityLogHandler.LogFormat(LogType.Log, null,
                        $"{prefix}{args?.LogLog?.Message}{(args?.LogLog?.Exception != null ? " Exception: " + args.LogLog.Exception.ToString() : "")}");
                }
            };
        }
        ...
        private static ILog GetLogger(Object context)
        {
            ...
            if (Loggers.TryGetValue(type, out var typedLog))
            {
                return typedLog;
            }
